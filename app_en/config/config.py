...
ENGLISH_VERSION_DIR = os.path.join(PROJECT_DIR, "app_en")
SPEECH_DIR = os.path.join(ENGLISH_VERSION_DIR, "data/speech")  # TODO【路径】:语音文件存放的路径
TEXT_PATH = os.path.join(ENGLISH_VERSION_DIR, "sampled_text/sampled_text.json")   # TODO【路径】:语音转文字结果存放的路径
...
MYSQL_USER = "zkyao"  # TODO: mysql账号
MYSQL_PASSWORD = "aa12345678"  # TODO: mysql密码
LOG_DIR = os.path.join(ENGLISH_VERSION_DIR, 'logs') # TODO【路径】:日志的目录
...
class Args:
    ...
    seq_labels_path = os.path.join(DATAFILE_PATH, 'intents_en.txt')
    token_labels_path = os.path.join(DATAFILE_PATH, 'slots_en.txt')

    bert_dir = os.path.join(MODEL_PATH, 'bert-base-uncased')
    save_dir = os.path.join(MODEL_PATH, 'checkpoints_en')
    # load_dir = os.path.join(MODEL_PATH, 'checkpoint_en_0521')
    load_dir = os.path.join('/dev_data_2/zkyao/code/Weld-GPT_demo/intent_and_slot/checkpoints_en_0528/checkpoint-5000')
