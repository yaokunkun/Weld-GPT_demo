...
from app.services.add_and_update_service import get_index_and_thickness, add, update

router = APIRouter()

@router.post("/add_and_update", response_model=str)
def add_and_update(method, material, thickness, diameter, params):
    ...
    if index_and_thickness is None or len(index_and_thickness) == 0:  # 如果返回结果为空
        paramIndex = 1
        return add(method, material, thickness, diameter, paramIndex, params)
    else:  # 如果返回结果不为空
        ...
        if len(candidate_paramIndex) != 0:  # 如果焊接厚度在刚才的返回结果里面
            ...
            return update(method, material, diameter, paramIndex, params)
        else:
